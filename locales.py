TRANSLATIONS = {
    'uz': {
        'welcome': 'Xush kelibsiz! Iltimos, menyudan tanlang:',
        'welcome_name': '👋 Xush kelibsiz, <b>{name}</b>! Iltimos, menyudan tanlang:',
        'about': 'Klinika haqida🏥',
        'services': 'Xizmatlar va Ish vaqti⏰',
        'review': 'Sharh qoldirish⭐',
        'change_lang': 'Tilni o\'zgartirish🌐',
        'change_name': 'Ismni o\'zgartirish✏️',
        
        'about_text': '🏥 <b>Kids Doctor Klinikasi</b>\n\nBizning maqsadimiz — farzandlaringiz sog\'lig\'ini asrash va ularga eng yaxshi tibbiy xizmatlarni taqdim etishdir! ✨\n\n📍 <b>Manzil:</b> Qo\'rg\'ontepa ko\'chasi 141, Humo mahallasi, Yangihayot tumani, Toshkent shahri. (Mo\'ljal: 66-poliklinika).\n📞 <b>Telefon:</b> +998 94 007 30 03',
        
        'services_text': '🩺 <b>Tibbiy xizmatlar ro\'yxati:</b>\n\n👨‍⚕️ Pediatr va bolalar nevropatologi\n🦷 Stomatolog (kattalar va bolalar uchun)\n👂 LOR / Otolaringolog (kattalar va bolalar uchun)\n🦴 Ortoped-travmatolog\n🤧 Allergolog va gastroenterolog\n💆‍♂️ Massaj (kattalar va bolalar uchun)\n🔬 <b>Diagnostika va davolash:</b> UTT (УЗИ), EEG, Laboratoriya, Fizioterapiya va Ozonoterapiya.\n\n🕰 <b>Ish vaqti:</b> 24/7 (Kechayu kunduz ochiq)',
        
        'choose_service': 'Qaysi xizmat haqida sharh qoldirmoqchisiz?',
        'service_pediatrician': '👨‍⚕️ Pediatr',
        'service_dentist': '🦷 Stomatolog',
        'service_ent': '👂 LOR',
        'service_orthopedist': '🦴 Ortoped',
        'service_allergist': '🤧 Allergolog',
        'service_massage': '💆‍♂️ Massaj',
        'service_diagnostics': '🔬 Diagnostika',
        
        'choose_rating': 'Xizmatni 1 dan 5 gacha baholang:',
        'leave_comment': 'Iltimos, sharhingizni yozib qoldiring:',
        'review_saved': 'Sharhingiz uchun rahmat!',
        'choose_language': 'Tilni tanlang / Choose language / Выберите язык:',

        'ask_first_name': 'Sharhlaringizni bemor ma\'lumotlariga moslashtirish uchun, iltimos, <b>haqiqiy ismingizni</b> kiriting:',
        'ask_surname': 'Rahmat! Endi, iltimos, <b>familiyangizni</b> kiriting:',
        'ask_new_first_name': '✏️ Iltimos, yangi <b>ismingizni</b> kiriting:',
        'ask_new_surname': 'Endi yangi <b>familiyangizni</b> kiriting:',
        'name_updated': '✅ Ismingiz yangilandi: <b>{first_name} {surname}</b>',
        'registration_complete': 'Rahmat, <b>{first_name} {surname}</b>! 👋\nEndi tilni tanlang:'
    },
    'en': {
        'welcome': 'Welcome! Please choose from the menu:',
        'welcome_name': '👋 Welcome, <b>{name}</b>! Please choose from the menu:',
        'about': 'About Clinic🏥',
        'services': 'Services & Hours⏰',
        'review': 'Leave a Review⭐',
        'change_lang': 'Change Language🌐',
        'change_name': 'Edit Name✏️',
        
        'about_text': '🏥 <b>Kids Doctor Clinic</b>\n\nOur goal is to protect your children\'s health and provide them with the best medical services! ✨\n\n📍 <b>Location:</b> Kurgan-Tepa Street 141, Mahalla Humo, Yangihayot District, Tashkent. (Landmark: Polyclinic No. 66).\n📞 <b>Phone:</b> +998 94 007 30 03',
        
        'services_text': '🩺 <b>List of Medical Services:</b>\n\n👨‍⚕️ Pediatrician & Pediatric Neurologist\n🦷 Dentist (Adult & Child)\n👂 ENT / Otolaryngologist (Adult & Child)\n🦴 Orthopedist-Traumatologist\n🤧 Allergist & Gastroenterologist\n💆‍♂️ Massage (Adult & Child)\n🔬 <b>Diagnostics & Treatment:</b> Ultrasound (USG), EEG, Laboratory, Physiotherapy, and Ozonotherapy.\n\n🕰 <b>Working Hours:</b> 24/7 (Open round the clock)',
        
        'choose_service': 'Which service would you like to review?',
        'service_pediatrician': '👨‍⚕️ Pediatrician',
        'service_dentist': '🦷 Dentist',
        'service_ent': '👂 ENT',
        'service_orthopedist': '🦴 Orthopedist',
        'service_allergist': '🤧 Allergist',
        'service_massage': '💆‍♂️ Massage',
        'service_diagnostics': '🔬 Diagnostics',
        
        'choose_rating': 'Please rate the service from 1 to 5 stars:',
        'leave_comment': 'Please write your comment:',
        'review_saved': 'Thank you for your review!',
        'choose_language': 'Choose language / Tilni tanlang / Выберите язык:',

        'ask_first_name': 'To match your feedback with patient records, please enter your <b>real First Name</b>:',
        'ask_surname': 'Thank you! Now, please enter your <b>Surname (Last Name)</b>:',
        'ask_new_first_name': '✏️ Please enter your new <b>First Name</b>:',
        'ask_new_surname': 'Now enter your new <b>Surname</b>:',
        'name_updated': '✅ Your name has been updated: <b>{first_name} {surname}</b>',
        'registration_complete': 'Thank you, <b>{first_name} {surname}</b>! 👋\nNow please choose your language:'
    },
    'ru': {
        'welcome': 'Добро пожаловать! Пожалуйста, выберите в меню:',
        'welcome_name': '👋 Добро пожаловать, <b>{name}</b>! Пожалуйста, выберите в меню:',
        'about': 'О клинике🏥',
        'services': 'Услуги и часы работы⏰',
        'review': 'Оставить отзыв⭐',
        'change_lang': 'Сменить язык🌐',
        'change_name': 'Изменить имя✏️',
        
        'about_text': '🏥 <b>Клиника Kids Doctor</b>\n\nНаша цель — забота о здоровье ваших детей и предоставление лучших медицинских услуг! ✨\n\n📍 <b>Адрес:</b> ул. Курган-Тепа, 141, Махалля Хумо, Янгихаётский район, г. Ташкент. (Ориентир: Поликлиника № 66).\n📞 <b>Телефон:</b> +998 94 007 30 03',
        
        'services_text': '🩺 <b>Список медицинских услуг:</b>\n\n👨‍⚕️ Педиатр и детский невропатолог\n🦷 Стоматолог (взрослый и детский)\n👂 ЛОР / Отоларинголог (взрослый и детский)\n🦴 Ортопед-травматолог\n🤧 Аллерголог и гастроэнтеролог\n💆‍♂️ Массаж (взрослый и детский)\n🔬 <b>Диагностика и лечение:</b> УЗИ, ЭЭГ, Лаборатория, Физиотерапия и Озонотерапия.\n\n🕰 <b>Часы работы:</b> 24/7 (Круглосуточно)',
        
        'choose_service': 'О какой услуге вы хотели бы оставить отзыв?',
        'service_pediatrician': '👨‍⚕️ Педиатр',
        'service_dentist': '🦷 Стоматолог',
        'service_ent': '👂 ЛОР',
        'service_orthopedist': '🦴 Ортопед',
        'service_allergist': '🤧 Аллерголог',
        'service_massage': '💆‍♂️ Массаж',
        'service_diagnostics': '🔬 Диагностика',
        
        'choose_rating': 'Пожалуйста, оцените услугу от 1 до 5 звезд:',
        'leave_comment': 'Пожалуйста, напишите ваш комментарий:',
        'review_saved': 'Спасибо за ваш отзыв!',
        'choose_language': 'Выберите язык / Choose language / Tilni tanlang:',

        'ask_first_name': 'Чтобы связать ваш отзыв с данными пациента, введите ваше <b>настоящее имя</b>:',
        'ask_surname': 'Спасибо! Теперь введите вашу <b>фамилию</b>:',
        'ask_new_first_name': '✏️ Введите ваше новое <b>имя</b>:',
        'ask_new_surname': 'Теперь введите вашу новую <b>фамилию</b>:',
        'name_updated': '✅ Ваше имя обновлено: <b>{first_name} {surname}</b>',
        'registration_complete': 'Спасибо, <b>{first_name} {surname}</b>! 👋\nТеперь выберите язык:'
    }
}

def _(text_key: str, lang: str = 'uz') -> str:
    return TRANSLATIONS.get(lang, TRANSLATIONS['uz']).get(text_key, text_key)
